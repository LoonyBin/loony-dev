...
- A long *backstop* timeout (`_DEFAULT_BACKSTOP`, 600s, in `claude_session.py`) on `open`/`send_turn` is a **liveness net only** — it trips when `claude` crashes before firing a hook, never as the primary signal. It is currently **hard-coded** (the `ClaudeSession` constructor accepts a `backstop_seconds` arg but no caller wires it to config); see the "Configuration & tunable constants" convention for the tier it belongs in.
...
## Configuration & tunable constants (no magic numbers)

Any numeric literal that **tunes runtime behavior** — a timeout, poll interval, backoff, retry count, size/length limit, TTL, or threshold — is subject to this convention. **Never write such a value as an inline literal in a function body** (`timeout=1800`, `range(3)`, `time.sleep(5)`). Every one is a **named module-level constant** and is classified into exactly one of three tiers:

- **Tier 1 — operator-facing (Click option).** Values an operator would plausibly tune per deployment: work-gating timeouts, concurrency caps, poll `interval`, discovery limits. Add a `@click.option(...)` in `cli.py` (this gives it a CLI flag + `LOONY_DEV_<CMD>_<KEY>` env var + config-file key via Click's `default_map`), read it from `config.settings[...]`, **and** add a commented line to `config.toml.example` under the right `[section]`.
- **Tier 2 — advanced (section-dict setting).** Reliability knobs an operator rarely touches (e.g. `claude_turn_timeout_seconds`, `heartbeat_stale_after`, `github.max_retries`). No CLI flag. Read via a section helper — `_worker_setting(key, default)` (coding.py), `gh_setting(key)` (github/client.py) — which resolves `config.settings["<section>"][key]` with the named constant as the fallback. **Still document it** (commented) in `config.toml.example`; the constant's default and the example's stated default must agree.
- **Tier 3 — internal invariant (intentionally not configurable).** Cadence/granularity/buffer sizes where a knob would be noise, not value (per the "don't overwhelm the operator with bells and knobs" rule): PTY dimensions, socket/ring buffer sizes, sub-second poll/debounce granularity, the 1s scheduler tick. Keep it a named constant **with a `# not configurable: <reason>` comment** so the choice is deliberate and reviewable, not an oversight.

Rules that follow from this:
- **Bugs to avoid, by category:** (A) a behavior-tuning literal with *no* config path is a **severe** bug — promote it to Tier 1 or 2. (B) choosing Tier 3 for something an operator would want to tune is a judgement call — err toward Tier 2 for anything that gates or reclaims work, toward Tier 3 for pure internal cadence. (C) a live Tier-1/2 key **missing from `config.toml.example`**, or an example key that **no code reads** (a phantom knob), is a **documentation bug** — `config.toml.example` is the single source of truth for what is configurable and must stay in lockstep with the code.
- When you add or change a Tier-1/2 key, update `config.toml.example` in the **same change**. When you delete one, remove its example line.
