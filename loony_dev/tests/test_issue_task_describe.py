...
def test_describe_contains_fork_safe_pr_command():
    ...
    assert "gh pr create --fill -R $(gh repo view --json nameWithOwner -q .nameWithOwner)" in description


def test_describe_does_not_contain_bare_gh_pr_create():
    task = IssueTask(_make_issue())
    description = task.describe()
    # Every occurrence of "gh pr create" must be accompanied by "-R"
    import re
    bare = re.search(r"gh pr create(?! --fill -R)", description)
    assert bare is None, f"Found bare 'gh pr create' without '-R' at position {bare.start()}"
